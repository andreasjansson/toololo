...
class Run:
    def __init__(
        ...
        max_iterations=50,
        message_logger: MessageLogger = default_message_logger,
    ):
        logger.info(
            f"Initializing Run with model={model}, {len(tools)} tools, max_iterations={max_iterations}"
        )
        self.client = client
        self.model = model
    ...
    async def initialize(self) -> None:
        ...
        logger.info(
            f"Initializing tool schemas for {len(self.compatible_tools)} functions"
        )
        try:
    ...
    async def _generate_outputs(self) -> AsyncIterator[Output]:
        ...
        try:
            ...
            for self.iteration in range(self.max_iterations):
                ...
                while attempt < max_attempts:
                    try:
                        ...
                        if self.tool_schemas:
                            ...
                            logger.debug(
                                f"Including {len(self.tool_schemas)} tools in request"
                            )
                        ...
                        if self.reasoning_max_tokens:
                            ...
                            logger.debug(
                                f"Including reasoning with max_tokens={self.reasoning_max_tokens}"
                            )

                        logger.info(
                            f"Making API call to {self.model} with {len(messages)} messages"
                        )
                        ...
                        logger.warning(
                            f"API status error on attempt {attempt + 1}/{max_attempts}: {e}"
                        )
                        logger.warning(
                ...
                logger.debug(
                    f"Iteration {self.iteration} - Content: {assistant_message_content[:100] if assistant_message_content else None}"
                )
                logger.debug(
                    f"Iteration {self.iteration} - Tool calls: {bool(message.tool_calls)}"
                )
                logger.debug(
                    f"Iteration {self.iteration} - Finish reason: {response.choices[0].finish_reason}"
                )

                # Yield reasoning content if present
                if hasattr(message, "reasoning") and message.reasoning:
                    output = ThinkingContent(message.reasoning)
                    self.message_logger.log_message(output)
                    yield output

                # Yield text content if present
                if assistant_message_content:
                    output = TextContent(assistant_message_content)
                    self.message_logger.log_message(output)
                    yield output
                    ...
                    for tool_call in message.tool_calls:
                        ...
                        try:
                            if tool_call.function.arguments.strip():
                                ...
                                logger.warning(
                                    f"Empty function arguments for {func_name}, using empty dict"
                                )
                                func_args = {}
                        except json.JSONDecodeError as e:
                            logger.error(
                                f"Invalid JSON in function arguments for {func_name}: {tool_call.function.arguments}"
                            )
                        ...
                        self.message_logger.log_message(tool_content)
                        yield tool_content
                        ...
                        if func_name in self.function_map:
                            ...
                            if "_toololo_message_logger" in param_keys:
                                func_args["_toololo_message_logger"] = self.message_logger

                            task = self._execute_function(func, **func_args)
                            tool_use_tasks.append(
                                (tool_call, task, original_func, True)
                            )
                        else:
                ...
                if tool_use_tasks:
                    ...
                    for i, (
                        ...
                        original_func,
                        is_task,
                    ) in enumerate(tool_use_tasks):
                        func_name = tool_call.function.name
                        logger.debug(
                            f"Processing tool call {i + 1}/{len(tool_use_tasks)}: {func_name}"
                        )
                        ...
                        if is_task:
                            try:
                                ...
                                logger.error(
                                    f"Tool call exception: {traceback.format_exc()}"
                                )
                            ...
                            logger.warning(
                                f"Tool call failed with error: {task_or_error}"
                            )
                            result_content = task_or_error
                            success = False

                        # Yield the tool result
                        output = ToolResult(success, original_func, func_name, result_content)
                        self.message_logger.log_message(output)
                        yield output
                ...
                logger.debug(
                    f"Iteration {self.iteration} completed, continuing to next iteration"
                )

                # If no tool calls, we're done
                if not tool_use_tasks:
                    logger.info(
                        f"No tool calls in response, task completed after {self.iteration + 1} iterations"
                    )
                    return
